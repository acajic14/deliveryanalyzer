...
def add_dhl_branding_to_excel(workbook, worksheet, title="DHL Route Analysis"):
    """Add DHL branding to Excel worksheets"""
    # DHL Colors
    ...
    worksheet['A2'] = title
    worksheet['A3'] = f"Generated: {datetime.now().strftime('%Y-%m-%d %H:%M')}"
    ...
    worksheet['A2'].font = Font(color="000000", bold=True, size=14)
    worksheet['A2'].alignment = Alignment(horizontal="center", vertical="center")
    
    worksheet['A3'].font = Font(color="666666", size=10)
...
def send_email_with_attachment(smtp_server, smtp_port, sender_email, sender_password, 
    ...
    try:
        ...
        dhl_body = f"""
DHL EXPRESS - Route Analysis Report
...
def send_route_reports(route_summary, specialized_reports, email_mapping, output_path, timestamp,
    ...
    for report_type, route_prefixes in email_routes.items():
        ...
        if not recipients.empty and report_type in specialized_reports:
            ...
            subject = f"DHL Daily Route Report - {report_type} ({datetime.now().strftime('%Y-%m-%d')})"
            body = f"""Dear Team,
...
def process_multiple_manifests(uploaded_files):
    ...
    <div style="background: linear-gradient(90deg, #FFCC00 0%, #D40511 100%); 
        ...
        <p style="color: white; margin: 0;">Files processed successfully</p>
    </div>
...
def create_specialized_report(manifest_df, route_prefixes, report_name, output_path, timestamp):
    ...
    if not filtered_data.empty:
        ...
        with pd.ExcelWriter(report_path, engine='openpyxl') as writer:
            report_data.to_excel(writer, index=False, startrow=4)
        ...
        with pd.ExcelWriter(report_path, engine='openpyxl') as writer:
            pd.DataFrame(columns=['MATCHED_ROUTE', 'HWB', 'CONSIGNEE_NAME', 'CONSIGNEE_ADDRESS', 'CONSIGNEE_ZIP']).to_excel(writer, index=False, startrow=4)
            workbook = writer.book
            sheet = writer.sheets['Sheet1']
            add_dhl_branding_to_excel(workbook, sheet, f"DHL {report_name} Route Details")
    return report_path
...
def generate_reports(
    ...
    # Route Summary with DHL Branding
    with pd.ExcelWriter(summary_path, engine='openpyxl') as writer:
        route_summary.to_excel(writer, sheet_name='Summary', index=False, startrow=4)
        workbook = writer.book
        ...
        if not route_summary.empty:
            add_target_conditional_formatting(sheet, 'J', 6, len(route_summary)+5)
        ...
        for prefix in route_prefixes:
            ...
            if not prefix_data.empty:
                ...
                try:
                    sheet_data.to_excel(writer, sheet_name=prefix, index=False, startrow=4)
                ...
                empty_df.to_excel(writer, sheet_name=prefix, index=False, startrow=4)
                prefix_sheet = writer.sheets[prefix]
                add_dhl_branding_to_excel(workbook, prefix_sheet, f"DHL {prefix} Route Details")
    ...
    # Create the special cases Excel file with proper structure
    wb = Workbook()
    ws = wb.active
    ws.title = "Special Cases"
    
    # Add DHL branding to special cases
    add_dhl_branding_to_excel(wb, ws, "DHL Special Cases Report")
    
    current_row = 5  # Start after DHL header
    ...
    if not threshold_special_cases.empty:
        ...
        for row_idx in range(6, len(threshold_cases_processed)+6):
    ...
    if threshold_special_cases.empty and multi_shipment_special.empty:
        ws.cell(row=5, column=1, value="No special cases found")
    ...
    if not multi_shipment_customers.empty:
        ...
        with pd.ExcelWriter(multi_shipments_path, engine='openpyxl') as writer:
            multi_shipment_customers.to_excel(writer, index=False, startrow=4)
            workbook = writer.book
            sheet = writer.sheets['Sheet1']
        ...
        with pd.ExcelWriter(multi_shipments_path, engine='openpyxl') as writer:
            pd.DataFrame(columns=['CONSIGNEE_NAME_NORM', 'ZIP', 'Shipment Count', 'Total Pieces']).to_excel(writer, index=False, startrow=4)
            workbook = writer.book
            sheet = writer.sheets['Sheet1']
            add_dhl_branding_to_excel(workbook, sheet, "DHL Multiple Shipments Report")
    ...
    with pd.ExcelWriter(matching_details_path, engine='openpyxl') as writer:
        matching_details.to_excel(writer, index=False, startrow=4)
        workbook = writer.book
        sheet = writer.sheets['Sheet1']
    ...
    for col_idx, header in enumerate(headers, 1):
        cell = ws_wth.cell(row=5, column=col_idx, value=header)
        cell.font = Font(bold=True)
    ...
    # If no data at all, add a placeholder
    if threshold_special_cases.empty and multi_shipment_special.empty:
        ws_wth.cell(row=6, column=1, value="No special cases found")
    ...
    if 'PCC' in manifest_df.columns:
        ...
        if not priority_pccs.empty:
            ...
            ws = wb.active
            add_dhl_branding_to_excel(wb, ws, "DHL Priority Shipments Report")
            ws.cell(row=5, column=1, value="No priority shipments found")
            wb.save(priority_path)
    else:
        wb = Workbook()
        ws = wb.active
        add_dhl_branding_to_excel(wb, ws, "DHL Priority Shipments Report")
        ws.cell(row=5, column=1, value="No PCC data available")
        wb.save(priority_path)

    return timestamp, route_summary, specialized_reports, multi_shipments_path

def main():
    # DHL Branding Configuration
    st.set_page_config(
        page_title="DHL Delivery Route Analyzer",
        page_icon="📦",
        layout="wide",
    ...
    <div class="main-header">
        <h1>🚚 DHL Delivery Route Analyzer</h1>
        <p style="text-align: center; color: white; margin: 0;">Excellence. Simply delivered.</p>
    </div>
    """, unsafe_allow_html=True)
    
    # Sidebar DHL Branding
    st.sidebar.markdown("""
    <div style="text-align: center; padding: 1rem; background-color: #FFCC00; border-radius: 5px; margin-bottom: 1rem;">
        <h3 style="color: #D40511; margin: 0;">📦 DHL EXPRESS</h3>
        <p style="margin: 0; font-size: 12px; color: #666;">Excellence. Simply delivered.</p>
    </div>
    """, unsafe_allow_html=True)
    ...
    if uploaded_files:
        ...
        if not route_summary.empty and 'Predicted Stops' in route_summary:
            ...
            <div class="dhl-metric">
                <h3 style="color: #D40511; margin: 0;">📊 Predicted SPR</h3>
                <h2 style="color: #D40511; margin: 0;">{predicted_spr:.1f}</h2>
                <p style="margin: 0; color: #666;">Average Predicted Stops</p>
            </div>
            """, unsafe_allow_html=True)
        else:
        ...
        <div style="background: linear-gradient(90deg, #FFCC00 0%, #D40511 100%); 
                    padding: 1rem; border-radius: 10px; text-align: center; margin: 1rem 0;">
            <h3 style="color: white; margin: 0;">🎉 DHL Route Analysis Complete!</h3>
            <p style="color: white; margin: 0;">All reports generated successfully</p>
        </div>
        """, unsafe_allow_html=True)
            ...
            with col_email1:
                if st.button("📤 Send DHL Route Reports", type="primary", key="send_emails"):
                    if sender_email and sender_password:
                        with st.spinner("Sending DHL reports..."):
        ...
        with col8:
            if os.path.exists(specialized_reports['KRA']):
                with open(specialized_reports['KRA'], "rb") as f:
                    st.download_button("KRA Details", f, f"DHL_KRA_details_{timestamp}.xlsx",
        ...
        with col9:
            if os.path.exists(specialized_reports['LJU']):
                with open(specialized_reports['LJU'], "rb") as f:
                    st.download_button("LJU Details", f, f"DHL_LJU_details_{timestamp}.xlsx",
        ...
        with col14:
            if os.path.exists(specialized_reports['KOP']):
                with open(specialized_reports['KOP'], "rb") as f:
                    st.download_button("KOP Details", f, f"DHL_KOP_details_{timestamp}.xlsx",
                                      help="KP1 routes")
            else:
    ...
    <div style="text-align: center; padding: 2rem; color: #666;">
        <p>Powered by <strong style="color: #D40511;">DHL Express</strong> | 
        Route Optimization System | 
        © 2025 DHL International GmbH</p>
        <p style="font-size: 12px; color: #999;">Excellence. Simply delivered.</p>
    </div>
    """, unsafe_allow_html=True)
